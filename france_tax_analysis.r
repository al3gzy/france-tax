...
ggplot(france_tax, aes(x = Region, y = `Taxe.d.habitation`, fill = Region)) +
  geom_boxplot() +
  theme_minimal() +
  labs(title = "Taxe d'Habitation by Region", x = "Region", y = "Taxe d'Habitation") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))


france_tax <- france_tax[order(france_tax$Population), ]
france_tax$cumulative_population <- cumsum(france_tax$Population) / sum(france_tax$Population)
france_tax$cumulative_tax <- cumsum(france_tax$`Taxe.d.habitation`) / sum(france_tax$`Taxe.d.habitation`)

ggplot(france_tax, aes(x = cumulative_population, y = cumulative_tax)) +
  geom_line() +
  labs(title = "Tax Progressivity Curve", x = "Cumulative Population Share", y = "Cumulative Tax Share")

ggplot(france_tax, aes(x = Population, y = `Taxe.d.habitation`, size = `Taxe.d.habitation` / Population)) +
  geom_point(alpha = 0.6) +
  labs(title = "Bubble Chart: Taxe d'Habitation vs Population",
       x = "Population", y = "Taxe d'Habitation", size = "Tax per Capita")